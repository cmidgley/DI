...
function evaluateNode({ node, ...rest }) {
    ...
    if (rest.typescript.isIdentifier(node)) {
    ...
    else if (((_d = (_c = rest.typescript).isTypeAssertionExpression) === null || _d === void 0 ? void 0 : _d.call(_c, node)) || rest.typescript.isTypeAssertion(node)) {
        ...
        return evaluateImportEqualsDeclaration();
    }
        ...
        return evaluateTypeAliasDeclaration();
    }
    else if (rest.typescript.isInterfaceDeclaration(node)) {
        return evaluateInterfaceDeclaration();
    }
...
function evaluateNodeWithArgument(options, arg) {
    ...
    if (rest.typescript.isGetAccessorDeclaration(node)) {
        return evaluateGetAccessorDeclaration({ node, ...rest }, arg);
    }
    else if (rest.typescript.isSetAccessorDeclaration(node)) {
        return evaluateSetAccessorDeclaration({ node, ...rest }, arg);
    }
    else if (rest.typescript.isPropertyAssignment(node)) {
        return evaluatePropertyAssignment({ node, ...rest }, arg);
    }
    else if (rest.typescript.isPropertyDeclaration(node)) {
        return evaluatePropertyDeclaration({ node, ...rest }, arg);
    }
        ...
        return evaluateEnumMember({ node, ...rest }, arg);
    }
    else if (rest.typescript.isShorthandPropertyAssignment(node)) {
        return evaluateShorthandPropertyAssignment({ node, ...rest }, arg);
    }
    else if (rest.typescript.isDecorator(node)) {
        return evaluateDecorator({ node, ...rest }, arg);
    }
    else if (rest.typescript.isSpreadAssignment(node)) {
        return evaluateSpreadAssignment({ node, ...rest }, arg);
    }
    else if (rest.typescript.isMethodDeclaration(node)) {
        return evaluateMethodDeclaration({ node, ...rest }, arg);
    }
    else if (rest.typescript.isArrayBindingPattern(node)) {
        return evaluateArrayBindingPattern({ node, ...rest }, arg);
    }
        ...
        return evaluateCatchClause({ node, ...rest }, arg);
    }
        ...
        return evaluateOmittedExpression();
    }
    throw new UnexpectedNodeError({ node, typescript: rest.typescript });
}
 ...
 */
function evaluate({ typeChecker, node, environment: { preset = "NODE", extra = {} } = {}, typescript = TSModule, logLevel = 0 /* SILENT */, policy: { deterministic = false, network = false, console = false, maxOps = Infinity, maxOpDuration = Infinity, io = {
    read: true,
    write: false
}, process = {
    exit: false,
...
export { EvaluationError, IoError, MaxOpDurationExceededError, MaxOpsExceededError, MissingCatchOrFinallyAfterTryError, ModuleNotFoundError, NetworkError, NonDeterministicError, NotCallableError, PolicyError, ProcessError, UndefinedIdentifierError, UndefinedLeftValueError, UnexpectedNodeError, evaluate };
